...
/*CREATE TABLE ecom.Cart (
    ...
    FOREIGN KEY (customer_id) REFERENCES ecom.Customers(customer_id),
...
/*INSERT INTO ecom.Customers (customer_id, first_name, last_name, email, address)
VALUES
...
/*INSERT INTO ecom.Products (product_id, name, description, price, stockQuantity)
VALUES
...
/*INSERT INTO ecom.Cart (cart_id, customer_id, product_id, quantity)
VALUES
(1, 1, 1, 2),
(2, 1, 3, 1),
(3, 2, 2, 3),
...
/*INSERT INTO ecom.Orders (order_id, customer_id, order_date, total_amount, shipping_address)
VALUES
...
/*INSERT INTO ecom.Order_Items (order_item_id, order_id, product_id, quantity, item_amount)
VALUES
(1, 1, 1, 2, 1600.00),
(2, 1, 3, 1, 300.00),
(3, 2, 2, 3, 1800.00),
...
/*SELECT * FROM ecom.Products 
WHERE product_id NOT IN (SELECT DISTINCT product_id FROM ecom.Cart);*/
-------------------------------------------------------------------------------------------

--16.Find Customers Who Haven't Placed Orders.---

/*SELECT *FROM ecom.Customers 
WHERE customer_id NOT IN (SELECT DISTINCT customer_id FROM ecom.Orders);*/
--------------------------------------------------------------------------------------------

--17.Calculate the Percentage of Total Revenue for a Product.---

/*SELECT p.product_id, p.name, 
       (SUM(oi.item_amount) * 100) / (SELECT SUM(total_amount) FROM ecom.Orders) AS revenue_percentage
FROM ecom.Order_Items oi
JOIN ecom.Products p ON oi.product_id = p.product_id
...
/*SELECT * FROM ecom.Customers 
WHERE customer_id IN (SELECT customer_id FROM ecom.Orders WHERE total_amount > 1000);*/